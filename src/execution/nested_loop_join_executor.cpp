...
auto NestedLoopJoinExecutor::InnerJoin(const Schema &schema, Tuple *tuple) -> bool {
  if (index_ != 0) {
    ...
    for (uint32_t j = index_; j < right_tuples_.size(); j++) {
      ...
      if (plan_->Predicate()
              .EvaluateJoin(&left_tuple_, left_child_->GetOutputSchema(), &right_tuples_[j],
  ...
  if (index_ == 0) {
    ...
    while (left_child_->Next(&left_tuple_, &left_rid_)) {
      // 遍历右表
      for (const Tuple &right_tuple : right_tuples_) {
        // 当前正在遍历第 0 条，下一条应该遍历第 1 条
        index_ = (index_ + 1) % right_tuples_.size();
        if (plan_->Predicate()
                .EvaluateJoin(&left_tuple_, left_child_->GetOutputSchema(), &right_tuple,
                              right_child_->GetOutputSchema())
                .GetAs<bool>()) {
